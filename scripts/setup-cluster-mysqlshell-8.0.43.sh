#!/bin/bash
# Setup MySQL InnoDB Cluster using MySQL Shell 8.0.43
...
if ! command -v mysqlsh &> /dev/null; then
    ...
    echo ""
    echo "Please install MySQL Shell 8.0.43:"
    echo "  Download from: https://dev.mysql.com/downloads/shell/"
    echo "  Or use: wget https://dev.mysql.com/get/Downloads/MySQL-Shell/mysql-shell-8.0.43-linux-glibc2.17-x86-64bit.tar.gz"
    exit 1
fi

MYSQLSH_VERSION=$(mysqlsh --version 2>&1 | grep -oP 'Ver \K[0-9]+\.[0-9]+\.[0-9]+' | head -1)
echo "MySQL Shell version: $MYSQLSH_VERSION"
echo "Target MySQL version: 8.0.43"
echo ""
